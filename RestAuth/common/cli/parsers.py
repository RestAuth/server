...
service_opt_parser.add_argument('--service', action=ServiceAction, help="Act as if %(prog)s was SERVICE.")

user_parser = ArgumentParser(add_help=False)
user_parser.set_defaults(create_user=False)
user_parser.add_argument('user', action=UsernameAction, help="The name of the user.")
...
parser.add_argument(
    ...
    help='Skip users completely if they already exist. If not set, passwords and properties are overwritten '
    'if their respective --overwrite-... argument is given.')
parser.add_argument(
    '--skip-existing-groups', action='store_true', default=False,
    help='Skip groups completely if they already exist. If not set, users and subgroups will be added to the '
    'list.')
parser.add_argument(
    '--using', default=None, metavar="ALIAS", help="Use different database alias. (UNTESTED!)")
